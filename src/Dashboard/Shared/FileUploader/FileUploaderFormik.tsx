import { Dropzone, ExtFile, FileMosaic, DropzoneProps } from "@files-ui/react";
...
/** Props for FileUploaderFormik component */
export interface IFileUploaderFormikProps<T extends FormikValues> {
  /** Label text for the file uploader */
  textLabel: string;
  /** Form field name */
  name: string;
  /** Formik instance */
  ...
  onChange?: (updatedFiles: Array<ExtFile>) => void;
}

export default function FileUploaderFormik<T extends FormikValues>({
  textLabel,
  name,
  formik,
  onChange,
  ...rest
}: IFileUploaderFormikProps<T> & DropzoneProps) {
  const { setFieldValue, values } = formik;
  const [loading, setLoading] = React.useState(false);

  const updateFiles = (incomingFiles: Array<ExtFile>) => {
    setLoading(true);
    setFieldValue(name, incomingFiles);
    setTimeout(() => {
      setLoading(false);
    }, 2000);
  ...
  const removeFile = (id: number | string | undefined) => {
    const updatedFiles =
      values[name]?.filter(
        (file: { id: string | number | undefined }) => file.id !== id
      ) || [];
        ...
        label={
          <div className="file-pond-file flex flex-col gap-2 justify-center items-center">
            <div className="icon-svg-icon">
              ...
              >
                <rect x="0.5" width="32" height="32" rx="8" fill="#4C6CB2" />
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
            ...
            <h2 className="text-font-dark">إرفق الملف</h2>
            <h2 className="text-font-gray">الصيغة المتاحة : JPG/PNG</h2>
          </div>
        }
        behaviour={rest.multiple ? "add" : "replace"}
        {...rest}
      >
        {loading ? (
